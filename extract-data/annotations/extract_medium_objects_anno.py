...
class VOCSmallObjects:
    def __init__(self, small_obj_size, big_obj_size, data_dir, split_small, split):
    ...
    def extract_medium_obj(self):
        ...
        medium_id_list_file = os.path.join(self.data_dir, 'ImageSets/Main/{0}.txt'.format(self.split_small))

        if os.path.exists(medium_id_list_file):
            print('Removing annotation ids file: ' + medium_id_list_file)
            os.remove(medium_id_list_file)

        id_file = open(medium_id_list_file, "w")

        annotations_medium_dir = self.data_dir + 'AnnotationsMedium/'
        # if os.path.exists(annotations_medium_dir):
        #     print('Removing medium annotation xmls.')
        #     shutil.rmtree(annotations_medium_dir)
        os.makedirs(os.path.dirname(annotations_medium_dir), exist_ok=True)
        ...
        for id_ in ids:
            medium_objects_num = self.check_example(id_)
            count += medium_objects_num
            if medium_objects_num != 0:
                id_file.write(id_ + '\n')
            elif os.path.exists(annotations_medium_dir + id_ + '.xml'):
                os.remove(annotations_medium_dir + id_ + '.xml')
...
small_objects = VOCSmallObjects(32, 64, data_dir_voc, 'trainval_medium', 'trainval')
small_objects.extract_medium_obj()